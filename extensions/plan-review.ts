...
if(!root)return;
const state={reviewMode:true,selected:null,hovered:null,annotations:[],blocks:[],composer:null,editingId:null,composerAnchor:null,composerMode:null,lastClick:null,statusTimer:null,recentId:null,recentTimer:null,pendingSnippet:null,pendingMarkId:null,pendingAnnotationId:null,selectionPopover:null};
...
function commentOnSelection(info){const id=crypto.randomUUID();wrapSelectionWithMark(info.range,id);try{window.getSelection().removeAllRanges();}catch(e){}state.pendingSnippet=info.text.slice(0,240);state.pendingMarkId=id;state.pendingAnnotationId=id;const preview=info.text.length>60?info.text.slice(0,60)+'…':info.text;openComposer(info.block,null,{title:'Comment on: “'+preview+'”'});}
...
mark.pi-review-mark{background:rgba(103,210,231,.18);color:inherit;border-radius:3px;padding:1px 2px;box-shadow:inset 0 -2px 0 rgba(103,210,231,.55);cursor:pointer;transition:background .15s ease,box-shadow .15s ease}
mark.pi-review-mark:hover{background:rgba(103,210,231,.32);box-shadow:inset 0 -2px 0 #67d2e7}
