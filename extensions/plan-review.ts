...
if(!root)return;
const state={reviewMode:true,selected:null,hovered:null,annotations:[],blocks:[],composer:null,editingId:null,composerAnchor:null,composerAnchorRect:null,composerMode:null,composerConversation:null,lastClick:null,statusTimer:null,recentId:null,recentTimer:null,pendingSnippet:null,pendingMarkId:null,pendingAnnotationId:null,selectionPopover:null,selectionTimer:null,selectionHighlightName:'pi-review-selection'};
...
function closeComposer(){if(state.composer){state.composer.remove();state.composer=null;}clearSelectionHighlight();state.editingId=null;state.composerAnchor=null;state.composerAnchorRect=null;state.composerMode=null;state.composerConversation=null;window.removeEventListener('resize',positionComposer);window.removeEventListener('scroll',positionComposer,true);}
...
function renderAnnotations(){syncReviewMeta();if(state.annotations.length===0){annotationsEl.innerHTML='<div class="pi-review-empty"><div class="pi-review-empty-icon">·</div><div>Click any block to ask a question or leave feedback.</div></div>';return;}annotationsEl.innerHTML='';state.annotations.forEach((annotation)=>{annotation.conversation=Array.isArray(annotation.conversation)?annotation.conversation:[];const item=document.createElement('article');item.className='pi-comment-card';if(annotation.id===state.recentId){item.dataset.recent='true';}const snippetHtml=annotation.textSnippet?'<div class="pi-comment-snippet">“'+escapeHtml(annotation.textSnippet.slice(0,140))+(annotation.textSnippet.length>140?'…':'')+'”</div>':'';item.innerHTML='<button type="button" class="pi-comment-anchor">→ '+escapeHtml(annotation.targetTitle||annotation.targetId||'General comment')+'</button>'+snippetHtml+'<p class="pi-comment-body">'+escapeHtml(annotation.comment)+'</p><div class="pi-comment-actions"><button type="button" class="pi-review-edit">Edit</button><button type="button" class="pi-review-ask">Ask Pi</button><button type="button" class="pi-review-remove">Remove</button></div>';const focusBlock=()=>{if(annotation.targetId){const target=document.querySelector('[data-review-id="'+CSS.escape(annotation.targetId)+'"]');if(target){target.scrollIntoView({behavior:'smooth',block:'center'});openComposer(target,annotation);}}};item.querySelector('.pi-comment-anchor')?.addEventListener('click',focusBlock);item.querySelector('.pi-review-edit')?.addEventListener('click',focusBlock);item.querySelector('.pi-review-ask')?.addEventListener('click',()=>{item.classList.toggle('pi-qa-open');const input=item.querySelector('.pi-qa-input');if(input&&item.classList.contains('pi-qa-open'))input.focus();});item.querySelector('.pi-review-remove')?.addEventListener('click',()=>{state.annotations=state.annotations.filter((entry)=>entry.id!==annotation.id);renderAnnotations();updateBadges();});item.appendChild(renderThread(annotation));if(annotation.conversation.length>0)item.classList.add('pi-qa-open');annotationsEl.appendChild(item);});}
function addGeneralComment(){const existing={id:crypto.randomUUID(),targetId:null,targetTitle:'General comment',comment:'',textSnippet:null};closeComposer();clearSelected();const drawer=document.createElement('div');drawer.className='pi-global-composer';drawer.innerHTML='<div class="pi-inline-composer-card"><div class="pi-inline-composer-header"><strong>General comment</strong><button type="button" class="pi-inline-close" aria-label="Close">×</button></div><textarea class="pi-inline-textarea" placeholder="General feedback about the document"></textarea><div class="pi-inline-actions"><div class="pi-inline-hint">⌘↵ save · Esc cancel</div><button type="button" class="pi-inline-save">Save</button></div></div>';document.body.appendChild(drawer);state.composer=drawer;state.composerMode='general';state.editingId=existing.id;const textarea=drawer.querySelector('.pi-inline-textarea');const close=()=>{closeComposer();};const save=()=>{const comment=textarea.value.trim();if(!comment){showStatus('Write a comment first.','error');return;}state.annotations.push({...existing,comment,conversation:[],createdAt:new Date().toISOString()});renderAnnotations();close();clearStatus();showStatus('General comment saved.','success');};drawer.querySelector('.pi-inline-save')?.addEventListener('click',save);drawer.querySelector('.pi-inline-close')?.addEventListener('click',close);textarea.addEventListener('keydown',(event)=>{if((event.metaKey||event.ctrlKey)&&event.key==='Enter'){event.preventDefault();save();}if(event.key==='Escape'){event.preventDefault();close();}});textarea.focus();}
function showSubmittedOverlay(count){const overlay=document.createElement('div');overlay.id='pi-submitted-overlay';overlay.innerHTML='<div class="pi-submitted-card"><div class="pi-submitted-check">✓</div><h2>Feedback submitted</h2><p>'+count+' '+(count===1?'comment':'comments')+' sent back to Pi.</p><p class="pi-submitted-hint">You can close this tab.</p><button type="button" id="pi-submitted-close">Close tab</button></div>';document.body.appendChild(overlay);const tryClose=()=>{try{window.close();}catch(e){}};document.getElementById('pi-submitted-close')?.addEventListener('click',tryClose);setTimeout(tryClose,400);}
...
function clearSelectionHighlight(){try{if(window.CSS?.highlights)CSS.highlights.delete(state.selectionHighlightName);}catch(e){}}
function setSelectionHighlight(range){clearSelectionHighlight();try{if(window.CSS?.highlights&&window.Highlight){window.CSS.highlights.set(state.selectionHighlightName,new Highlight(range.cloneRange()));}}catch(e){}}
function commentOnSelection(info){const rect=info.range.getBoundingClientRect();const selectionRange=info.range.cloneRange();try{window.getSelection().removeAllRanges();}catch(e){}state.pendingSnippet=info.text.slice(0,240);state.pendingMarkId=null;state.pendingAnnotationId=crypto.randomUUID();state.lastClick={x:rect.left+Math.min(rect.width/2,220),y:rect.bottom};openComposer(info.block,null,{title:info.block.dataset.reviewTitle||info.block.dataset.reviewId||'Selection',snippet:info.text,anchorRect:rect,selectionRange});}
...
::highlight(pi-review-selection){background:rgba(250,204,21,.32);color:inherit}
