...
function wrapSelectionWithMark(range,id){const mark=document.createElement('mark');mark.className='pi-review-mark';mark.dataset.annotationId=id;try{range.surroundContents(mark);return mark;}catch(e){const frag=range.extractContents();mark.appendChild(frag);range.insertNode(mark);return mark;}}
function commentOnSelection(info){const id=crypto.randomUUID();wrapSelectionWithMark(info.range,id);try{window.getSelection().removeAllRanges();}catch(e){}state.pendingSnippet=info.text.slice(0,240);state.pendingMarkId=id;state.pendingAnnotationId=id;openComposer(info.block,null,{title:info.block.dataset.reviewTitle||info.block.dataset.reviewId||'Selection',snippet:info.text});}
